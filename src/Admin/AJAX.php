...
class AJAX {
	...
	public function hide_admin_notice() {
		...
		if ( isset( $_POST['software'] ) && preg_match( '/(?:ssl|https)/', sanitize_key( $_POST['software'] ) ) ) {
	...
	public function wp_auto_update() {
		...
		if ( preg_match( '/^(?:minor|major|disabled|dev)$/', sanitize_key( $_POST['wp_auto_update'] ) ) ) {
			supv()->core()->updates->set_core_auto_update_option( sanitize_key( $_POST['wp_auto_update'] ) );
		}
